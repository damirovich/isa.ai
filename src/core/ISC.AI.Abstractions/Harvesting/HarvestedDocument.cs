...
/// Подразделение. Сборщик его НЕ заполняет (с 2026-08-19): он работает вне контура и справочника
/// подразделений не видит — подразделение выбирает оператор при импорте пакета внутри контура, и его
/// выбор перекрывает это поле. Поле сохранено ради чтения старых пакетов; <see langword="null"/> без
/// выбора при импорте → отказ (fail-closed, ТБ-024).
/// </param>
