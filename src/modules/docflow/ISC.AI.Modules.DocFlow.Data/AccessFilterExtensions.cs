...
///
/// Решётка обязана СОВПАДАТЬ с floor'ом ядра (<c>BaselineAccess</c> в <c>ISC.AI.AI</c>) и проверкой
/// раздачи файлов (этап 4.3) — это один и тот же инвариант в трёх слоях; <c>BaselineAccess</c>
/// не переиспользован намеренно (модуль на тот проект не ссылается, а по конкретному типу предикат
/// пишется прямо в LINQ). Сужающая политика (второй Where) — построчные правила профиля по роли/
/// владению (§2.1 ТЗ СКИД, ADR-0014): без профиля с политикой <c>BuildFilter</c> пропускает всё
/// без изменений.
/// </remarks>
