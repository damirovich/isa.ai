...
public class DocumentComment : SoftDeletableEntity
{
    ...
    /// <summary>
    /// Версия файла документа, действовавшая в момент публикации комментария; <see langword="null"/> —
    /// файла тогда не было.
    /// </summary>
    /// <remarks>
    /// СНИМОК, а не ссылка «на текущую»: замечание «в пункте 3 ошибка» относится к ТОМУ тексту,
    /// который автор читал. Файл документа версионируется (§3.3), и после замены новой версией
    /// комментарий без этой отметки начинал бы указывать не туда — читатель ищет пункт 3 в новой
    /// редакции и не находит, либо находит другой. Значение не меняется никогда: у комментария
    /// нет истории правок, и «переехать» на новую версию он не может.
    /// Слабая ссылка по значению — файл может быть удалён, а комментарий обязан остаться.
    /// </remarks>
