...
/// <remarks>
/// Право администратора здесь НЕ спрашивается намеренно: пароль меняет владелец учётной записи, а
/// подтверждением полномочия служит знание текущего пароля (проверяет порт ядра) — владение сессией
/// его не заменяет, иначе перехваченная сессия позволила бы сменить пароль и закрепиться (ТБ-016).
/// </remarks>
    ...
    : IRequest<ResponseDto<bool>>, IAuditableRequest
{
    ...
    public string? AuditSummary => "admin:account:change-own-password";
