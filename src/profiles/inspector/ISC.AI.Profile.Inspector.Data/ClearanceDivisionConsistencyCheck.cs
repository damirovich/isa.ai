...
/// которых НЕТ в справочнике профиля (<c>inspector.division</c>).
/// </summary>
/// <remarks>
/// Словарь номеров подразделений ОБЯЗАН быть общим для решётки доступа ядра и справочника профиля
/// (см. <c>IDivisionDirectory</c>), но ничем не проверяется: обе стороны наполняются независимо, а на
/// боевом контуре допуски исторически правились SQL'ем. Расхождение не даёт ни ошибки, ни отказа —
/// оно проявляется у пользователя как «мне ничего не видно» и «не могу зарегистрировать документ,
/// хотя допуск выдан», то есть в самой неудобной для диагностики форме. Проверка переводит это в
/// явное предупреждение при старте; тот же разбор показывает экран «Допуски пользователей».
///
