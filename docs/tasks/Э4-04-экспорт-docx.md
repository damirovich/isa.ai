...
|---|---|
| Этап | Э4 — MVP (P0) |
| Статус | ✅ Выполнено (порт+OpenXml-экспортёр+use-case+аудит+тесты + UI-кнопка скачивания; сборка 0/0) |
...
- ✅ UI: кнопка «Экспорт в .docx» на странице Генератора (скачивание через JS-interop `downloadFileFromStream`).
...
## Осталось
- Живой клик-тест скачивания (front-end проверяется только в браузере).
